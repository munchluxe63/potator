...
#ifdef _TINSPIRE
void screen_showchar(int x, int y, unsigned char a, const int fg_color, const int bg_color) 
#else
...
void print_string_video(short x, const short y, const char *s) 
{
	...
	{
#ifdef _TINSPIRE
		screen_showchar(x, y, s[i], (255 >> 3) << 11 	| (0 >> 2) << 5 | (0 >> 3), 0);
#else
...
/*void menuSaveBmp(void) 
{
	...
	if (cartridge_IsLoaded()) 
	{
		snprintf(szFile, sizeof(szFile), "./%s",strrchr(gameName,'/')+1);
